...
df = pd.read_csv(f'data/{ticker}.txt', names=['Date', 'Open', 'High', 'Low', 'Close', 'Volume'], header=None)
df
#%%
df=df[['Close']]
...
df2 = df.iloc[index1 - 5000:index1]
df2 = df2.reset_index(drop=True)
df2
#%%
plt.figure(figsize=(14, 7))
plt.plot( df['Close'], label='Stock Price')
# plt.plot(df2['Date'], df2['SMA50'], label='SMA 50')
# plt.plot(df2['Date'], df2['SMA200'], label='SMA 200')
# plt.plot(df2['Date'], df2['SMA1000'], label='SMA 1000')
plt.xlabel('Date')
...
#%%
df2=reset_index(df2)
df2=df2[['Close','SMA50','SMA200','SMA1000']]
df2
# %%
...
for iii in range(40):
    ...
    stock_data = df2.iloc[i - BACKS:i].values.flatten().tolist()
    stock_data
    ...
    for index1, s, ns, a, immediate, done, v_hat, prob_old, Gt, A in all_list[:-1]:
        ...
        if a_trained==2: # sell
            ...
            position+=1
        price_now=df2['Close'][index1]
        stock_data=df2.iloc[index1-BACKS:index1].values.flatten().tolist()
        price_next=df2['Close'][index1+1]
...
# Price with Actions
plt.figure(figsize=(14, 7))
plt.plot(df2['Close'].values, label='Price')
buy_signals = np.where(np.array(action_list) == 1)[0]
sell_signals = np.where(np.array(action_list) == 2)[0]
plt.scatter(buy_signals, df2['Close'].values[buy_signals], marker='^', color='g', label='Buy', alpha=0.6)
plt.scatter(sell_signals, df2['Close'].values[sell_signals], marker='v', color='r', label='Sell', alpha=0.6)
plt.xlabel('Time Step')
plt.ylabel('Price')
